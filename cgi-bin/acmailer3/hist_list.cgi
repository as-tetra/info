...
foreach my $ref (@DATA) {
	...
	} elsif ($row->{send_type} eq "2") {
		$row->{disp_send_type} = "バックグラウンド処理";
	} elsif ($row->{send_type} eq "0") {
...
for(1..$pagenumall){
	if($_ == $page){
		...
		$data_ref->{pagelink} .= "｜<A href=\"hist_list.cgi?page=$_&dispnum=$dispnum$data_ref->{search_url}\">$_</A>";
	}
}
